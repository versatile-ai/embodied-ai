...
> 本文记录的是 2026-09-17 至 2026-09-18 在 `192.168.0.192` 上进行的
> 历史部署与仿真试验，不能作为当前 π0.5 服务的连接说明。

部署日期：2026-09-17。目标：`192.168.0.192`（`aura-0916-3.novalocal`），经 `root@159.138.11.11` 跳转。

## 当前生效方案

按用户批准，仿真在本地 `simulation/mujoco` 运行并监听 `127.0.0.1:8763`。当前 π0.5 推理服务运行在 **`192.168.0.155`（SSH 别名 `npu155`）**，经本机 SSH 配置的跳板 `119.8.234.170` 访问。远端 192 的 MuJoCo 和 π0.5 记录均为历史资料，不是当前直播或推理的数据来源。

本地启动：在 `simulation/mujoco` 执行 `.\.venv\Scripts\python.exe start_server.py`。当前远程推理的端口转发：

```powershell
ssh -N -o ExitOnForwardFailure=yes -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -L 127.0.0.1:8642:127.0.0.1:8642 npu155
```
...
历史 192 环境的连接结论不适用于当前 155 服务；当前部署状态和健康契约以 `simulation/npu/pi05_service.py` 及其运行时 `/health` 返回为准。

## 部署边界

初始部署按照 `pi05_colleague_guide.md` 的分工，192 曾承载 π0.5 昇腾推理服务。2026-09-17 后续按用户要求增加独立 MuJoCo 仿真容器：物理、三路相机渲染和录像也在 192 执行。下文内容均为该历史环境记录。
